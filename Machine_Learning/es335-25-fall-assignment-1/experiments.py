...
np.random.seed(42)
num_average_time = 35  # Number of times to run each experiment to calculate the average values
...
def plot_time_complexity(N_values: list[int], M_values: list[int], fit_times: np.ndarray, predict_times: np.ndarray,  task: str, criterion: str, percent_real=0.5):
    N_grid, M_grid = np.meshgrid(M_values, N_values)

    fig = plt.figure(figsize=(14, 6))

    ax1 = fig.add_subplot(121, projection='3d')
    print(fit_times,predict_times)
    ...
    # rather than showing, save the plots
    plt.savefig(f'time_complexity_{task}_{criterion}_{percent_real}.png')
    plt.close()
...
def run_experiments(task: str, percent_real=0.5):
    ...
    plot_time_complexity(N_values, M_values, fit_times, predict_times, task, criterion,percent_real)
